...
def power_up(io, io_group, pacman_version, ramp, tile, vdda_dac, vddd_dac, \
             verbose=False, reset_length=600000000, vdda_step=1000, \
             vddd_step=1000, ramp_wait=30, warm_wait=20):
    ...
    if ramp==True:
        ...
        for i in tile:
            if pacman_version=='v1rev4':
                ...
                if verbose: print('WARNING: PACMAN version ',pacman_version,' unknown')
                return
            bits[-1*i]='1'
        io.set_reg(0x00000010, int("".join(bits),2), io_group=io_group)
        for i in tile:
            ctr=0; vdda=0
            if verbose: print('Tile ',i,' VDDA DAC: ',vdda_dac[i-1])
            ...
            if vdda>=vdda_dac[i-1]:
                if verbose: print('Tile ',i,': ',time.time()-start,\
                                  ' s ramping VDDA ', vdda)
        if verbose:
        ...
        for i in tile:
            ctr=0; vddd=0
            if verbose: print('Tile ',i,' VDDD DAC: ',vddd_dac[i-1])
